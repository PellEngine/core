...
void Window::terminateSwapChain() {
  ...
  vkDestroyRenderPass(instance->getDevice(), transparentRenderPass, nullptr);
...
/*
  ---------------------------
  Single exec command buffers
  ---------------------------
*/
VkCommandBuffer Window::startCommandBuffer() {
...
void Window::endCommandBuffer(VkCommandBuffer commandBuffer) {
  ...
  vkFreeCommandBuffers(instance->getDevice(), commandPool, 1, &commandBuffer);
}
